...
class TrackingOrderMsrmtController extends Controller {

	public function index(Request $request)
	{
	    $subQuery = TrackingOrderMsrmt::selectRaw('MAX(entity_id) as max_id')
	        ->groupBy('unique_id');
	    $sort='';
	    // Base query that applies common filters
	    $baseQuery = TrackingOrderMsrmt::whereIn('entity_id', $subQuery)
	        /*->where(function ($q) {
	        ...
	        ->whereIn('order_item_status', ['Invoiced']);
	        //->where('refund', 0); 
...
$count_dispatch = $count_expedite = $count_express_shipping =$count_hold =0;
    ...
    public function export(Request $request)
	{
	    ...
	    $query = TrackingOrderMsrmt::leftJoin('datewise_order_report_remark as dorr', 'appsheet_to_inhouse.unique_id', '=', 'dorr.item_sku')
