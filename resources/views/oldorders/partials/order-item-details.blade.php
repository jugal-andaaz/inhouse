@php
    ...
    if(isset($_GET['sku'])){
        $products = getOldItemsByOrderId($increment_id,$_GET['sku']); 
    }else{
        $products = getOldItemsByOrderId($increment_id);
    }
            ...
            <tr class="d-block d-md-table-row {{ $refunded }} {{ $styeleHide }}">
                ...
                <td class="statement-td d-block d-md-table-cell mb-3 td img" data-label="@lang('#')" style="width: 18%;">
                    <div class="card">
                        ...
                        @if(Str::contains($product->product_img, "img src="))
                            {!! $product->product_img !!}
                        @else
                            <img src="{{ Str::contains($product->product_img, 'https://assets2.andaazfashion.com') ? $product->product_img : asset('https://new.inhouse.andaazfashion.com'.str_replace('~','',$product->product_img)) }}" class="img-fluid me-2 mb-2 mb-md-0" alt="{{ $product->product_name }}" />
                        @endif
                        </div>
                    </div>
                </td>
                <td class="perticular" data-label="@lang('Particulars')" style="width:22%;">
                    <div class="card border-0 shadow-none m-0 p-0">
                        <div class="d-grid gap-1 my-2 px-1">
                            <table class="table table-borderless table-striped product-tbl-font">
                                <tbody>
                                    <tr>
                                     ...
                                     <tr>
                                        <td><strong>Received Status</strong><span style="float: right;">:</span></td>
                                        <td>{!! !empty($product->rr_store_status) ? $product->rr_store_status : '---' !!}</td>
                                    </tr>
                                    <tr>
                                        <td><strong>Warehouse Status</strong><span style="float: right;">:</span></td>
                                        <td>{!! $product->product_factory_status !!}</td>
                                    </tr>
                                    <tr>
                                        <td><strong>Priority</strong><span style="float: right;">:</span></td>
                                        <td>{!! $product->flag_text !!}</td>
                                    </tr>
                                    <tr>
                                        <td><strong>Occassion Priority</strong><span style="float: right;">:</span></td>
                                        <td>{!! !empty($product->festival_delivery_status) ? $product->festival_delivery_status : '---' !!}</td>
                                    </tr>
                                    <tr>
                                        <td><strong>Dispatch Date</strong><span style="float: right;">:</span></td>
                                        <td>{{ $product->product_dispatch_date }}</td>
                                    </tr>
                                    <tr>
                                        <td><strong>Occassion Date</strong><span style="float: right;">:</span></td>
                                        <td>{!! !empty($product->occassion_date) ? $product->occassion_date : '---' !!}</td>
                                    </tr>
                                </tbody>
                            </table>
                            <?php /* andaaz_order_log Data with filter By column_name=Factory Status* / ?>
                            ...
                            <a style="display: none;" href="{{ route('order.item.logs', ['item_id' => $product->id, 'proditem_id' =>$product->product_item_id, 'order_id' =>$order->increment_id]) }}" onclick="window.open(this.href, 'popupWindow', 'width=900,height=650,scrollbars=yes'); 
                                return false;"><button class="btn btn-primary p-2 mb-2">OLD Item Log</button>
                            </a> */ ?>
                        </div>
                    </div>
                </td>
                <td class="statement-td d-block d-md-table-cell mb-3 w-15 sku" style="width: 8%;">
                    ...
                    <p><strong>Tracking ID: </strong> {{ $product->product_tracking }}</p>
                    <p><strong>Courier: </strong> {{ $product->product_courier }}</p> 
                ...
                <td class="statement-td d-block d-md-table-cell mb-3 w-20 td" style="width: 30%;">
                        ...
                        <table class="table table-bordered table-striped product-tbl-font">
                            ...
                            @foreach($productSizeCollection as $productSizeCollect)
                                @php
                                    ...
                                    $standardMeasur = getOldStandardMeasurementByItemId($product->id);
                                    ...
                                    $measurementCollect = getOldMeasurmentByOrderId($measurementValid);
                                ...
                                @if($measurementValid < 1 && isset($productSize[0]) && isset($productSize[1]))
                                    <tbody>
                                        <tr>
                                            <td class="">{!! $productSize[0] !!}</td>
                                            <td class="" colspan="2">{!! $productSize[1] !!}</td>
                                        </tr>
                                    </tbody>
                                @endif
                                ...
                                @if(count($productSize) == 2)
                                    ...
                                    if(trim($productSize[0])=='Tailoring Service' || trim($productSize[0])=='Select Tailoring Service'){
                                        ...
                                        $productmeasure['tailoringService'] = trim($productSize[1]);$coupleDetailsVal = 0;
                                    } else if(trim($productSize[0])=='Bust Size' || trim($productSize[0])=='Andaaz Size' || trim($productSize[0])=='Body Chest Size'){
                                        $productmeasure['andaazSize'] = trim($productSize[1]);$coupleDetailsVal = 0;
                                    } else if(trim($productSize[0])=='Body Height'){
                            ...
                            @if (trim($productSize[0]!=='Size')) 
                                @if($measurementValid > 0)
                                    @include('oldorders.partials.measurements',['measurementId' => $measurementValid])
                                @else
                                    @php 
                                        $productSizes = str_replace('"','',$product->product_size);
                                        $productSizeCollection = explode('|', $productSizes);
                                    ...
                                    <tbody>
                                        @if ($standardMeasur)
                                           @include('oldorders.partials.measurement_readysize',['product_id' => $product->id])
                                        <?php /*@else
                                            @if (((strpos($productSizeCollect, 'Womens') === false) || (strpos($productSizeCollect, 'Mens') === false)) && $coupleDetailsVal==0) 
                                                @if(!empty($productmeasure['andaazSize']))
                                                <tr>
                                                    <td>Andaaz/Bust Size</td>
                    ...
                    @include('oldorders.partials.specialinstruction') 
                    @include('oldorders.partials.addons',['product_size' => $product->product_size]) 
                        @php
                            $images = getProductImagesByPid($product->id);
                            $cnt=1;
                        @endphp 
                        <table>
                            <tbody>
                                @foreach($images as $imageRemark)
                                <tr>
                                    <td>{{ $cnt }}</td>
                                    <td>{{ $imageRemark->remark }}</td>
                                    <td>
                                        <img src="{{ Str::contains($imageRemark->path, 'https://assets2.andaazfashion.com') ? $imageRemark->path : asset('https://inhouse.andaazfashion.com/uploads/olduploads/uploads'.str_replace('~/Uploads','',$imageRemark->path)) }}" class="img-fluid me-2 mb-2 mb-md-0" alt="{{ $imageRemark->name }}" />
                                    </td> 
                                </tr>
                                @php
                                    $cnt++;
...
<?php /* @include('orders.partials.add-measurement-pop')*/ ?>
<style type="text/css">
    ...
    @foreach($epsplTracking as $epsplTrack)
        <div class="epspl-wrap mt-3 mb-3">
            ...
            <div class="epspl-infobar">
                ...
                <div class="epspl-info-item">
                    <span class="epspl-label">SKU:</span>
                    <span class="epspl-value">{{ $epsplTrack->product_sku }}</span>
                </div>
                <div class="epspl-info-item">
